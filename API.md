...
| 优先级 | 规则 | 例 |
|---|---|---|
| 1 | 关键词后跟纯数字 | `您的验证码是 482913` |
| 2 | 关键词后跟字母数字混合码 | `Your code: A3F9K2` |
| 3 | 纯数字后紧跟关键词 | `482913 is your verification code` |
| 4 | 裸的 4–8 位数字（兜底） | 主题就是一串数字 |

**规则优先于字段**：先用最强的规则扫遍主题、正文、摘要与 HTML，再降级到下一条。
因此正文里带关键词的验证码会胜过主题里的订单号——这是单条正则最常见的错法。

HTML 会先去掉标签再匹配，`script` 与 `style` 整块丢弃，避免撞上样式里的数字。

关键词覆盖：验证码、校验码、动态码、安全码、code、verification code、OTP、PIN 等。
模板特殊时仍可传自定义正则，它按原样生效，不受这组规则影响。

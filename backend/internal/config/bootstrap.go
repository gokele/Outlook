...
# ---- 反向代理：谁有资格代填「来源 IP」----
#
# 只有当连接本身来自这里列出的地址时，X-Forwarded-For / X-Real-IP 才会被采信。
#
# 这一条是两个安全控制的地基：API Key 的 IP 白名单，以及登录失败限速。
# 少了它，任何人加一个请求头就能冒充白名单里的 IP，
# 或者每次换一个伪造 IP 让登录限速永远数不满。
#
# 默认只信本机回环，对应「Nginx 与本服务同机」这种最常见的部署方式。
# 反代在另一台机器上时，把那台机器的地址填进来（IP 或 CIDR，逗号分隔）。
# 本服务直接对外、前面没有任何反代时，留空最安全。
